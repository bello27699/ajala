...
import { sendEmail } from "../../../utils/helpers/email"; dotenv.config();
...
export class FlightRepositoryImpl implements FlightRepository {
  ...
  async getPaymentInvoiceWithGUID(guid: any): Promise<ResponseModel> {
    ...
    console.log(invoiceData); return send200("Successfully fetched invoice", invoiceData.data.getInvoice);
  }
  ...
  async handleWebHook(event: any): Promise<void> {
    ...
    if (event.event == TiqwaEvents.TICKET_ISSUED) {
      await sendEmail(
        ...
        `Thank you for booking with Ajala DTMS. Here are your booking details
        <br><br>Passenger Name:
        ${event.data.passengers[0].title} ${event.data.tickets[0].traveler}
        <br>PNR: ${event.data.pnr}\n <br> Flight Agent: Tiqwa
        <br> Airline: ${event.data.outbound[0].airline_details.name}
        <br>Phone No: ${event.data.passengers[0].phone_number}
        <br>Amount paid: NGN ${event.data.amount}\n
        <br>Reference: ${event.data.reference}
        <br>Ticket number: ${event.data.tickets[0].ticket_number}
        <br>From: ${event.data.outbound[0].airport_from}
        <br> Depature Time: ${event.data.outbound[0].departure_time}
        <br>To: ${event.data.outbound[0].airport_to} <br>Cabin Type: ${event.data.outbound[0].cabin_type}`
      );
    }
  ...
  async confirmPrice(flight_id: string): Promise<ResponseModel> {
    try {
      const response = await fetch(
        ...
        tiqwaConnect("GET", {})
      ); const { status } = response; const data = await response.json(); return genericResponse(status, data.message, data);
    } catch (error: any) {
      return send400(error.message, null);
    }
  }
